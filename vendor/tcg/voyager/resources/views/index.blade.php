...
$year_count = DB::table('customers')
    ->whereYear('created_at','2018')
    ->get();
$count_year = 1;
foreach ($customers as $customer){
    if(date_format($customer->created_at,'Y')!='2018'){
        $count_year++;
    }
}
$year = 2018;
        ...
        <div class="row">
            <div class="col-lg-3 col-md-3">
                            <div class="card-counter success">
                                ...
                                <span class="count-numbers" style="color: #fff;">Customer</span>
                                <span class="count-name" style="color: #000; font-size: 20px;">{!! $customer_count !!}</span>
                            </div>
            </div>
            <div class="col-lg-9 col-md-9">
                <div class="page-content">
                    <h2 class="container alert alert-info">The last deals</h2>
                    @for($j=$count_year;$j>=1;$j--)
                        <h2 class="container alert alert-info">These are the months of {!! $year !!}</h2>
                        @for($i=12;$i>=1;$i--)
                            <div class="container">
                                <?php $customer=DB::table("customers")
                                    ->whereMonth('created_at', '=', $i)->whereYear('created_at','=',$year)
                                    ->get();
                                $count = count($customer);
                                ?>
                                <button type="button" class="btn btn-info" style="width:300px;" data-toggle="collapse" data-target="#demo{!!$i!!}">{!! date("F",mktime(0,0,0,$i,1,2011))!!} {!! $year !!} ({!! $count !!})</button>
                                        ...
                                        @if(isset($customer))
                                            @foreach($customer as $customer)
                                                <tr>
                                                    ...
                                                    <td title="profile {!! $customer->name !!}"><a href="{!! route('voyager.customers.profile',['id'=>$customer->id]) !!}">{!! $customer->name !!}</a></td>
                                                    <td>@if($customer->web_site)<a  href="{!! $customer->web_site !!}"><button class="btn btn-success">click here</button></a> @endif</td>
                                                    <td><a  href="{!! route('voyager.customer.briefing',['id'=>$customer->id]) !!}"><button class="btn btn-danger">Briefing</button></a></td>
                                                    <td>{!! $customer->status !!}</td>
                                                    <td>{!! $customer->paket !!}</td>
                                                    <td>{!! $customer->seller !!}</td>
                                                    <td>{!! $customer->booker !!}</td>
                                                </tr>
                                            @endforeach
                                        @endif
                                        </tbody>
